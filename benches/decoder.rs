...
fn decoder_benchmarks(criterion: &mut Criterion) {
    ...
    group.bench_function("owned_one_pass", |bench| {
        bench.iter(|| decode_tcp_frame(black_box(&frame)).unwrap())
    });
    group.finish();

    let large_frame = encode_codec12_command(&vec![0x55; 48 * 1024]).unwrap();
