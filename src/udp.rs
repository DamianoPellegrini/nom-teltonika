...
use crate::{
    decoder_impl::decode_udp_datagram_with_limits,
    encoder_impl::encode_udp_ack,
    protocol_impl::{UdpDatagram, UdpDecodeError, UdpLimits},
};

#[derive(Debug)]
#[non_exhaustive]
/// Failure receiving, decoding, or acknowledging a UDP datagram.
pub enum UdpReceiveError {
    ...
    Decode(UdpDecodeError),
    /// The underlying socket operation failed.
    Io(io::Error),
}

impl fmt::Display for UdpReceiveError {
...
impl<S> TeltonikaUdpSocket<S> {
    /// Wraps a socket using [`UdpLimits::default`].
    pub fn new(socket: S) -> Self {
        Self::with_limits(socket, UdpLimits::default())
    }
    ...
    pub fn with_limits(socket: S, limits: UdpLimits) -> Self {
        Self {
            socket,
            limits,
            receive: vec![0; limits.max_datagram_bytes().saturating_add(1)],
        }
    }
...
impl TeltonikaUdpSocket<std::net::UdpSocket> {
    ...
    ///
    /// Unlike the slice decoder, this method requires the complete socket
    /// datagram to contain exactly one Teltonika message.
    ///
    /// # Errors
    ///
    /// Returns [`UdpReceiveError::Truncated`] if the datagram exceeds the receive
    /// limit, [`UdpReceiveError::Decode`] for protocol failures, and
    /// [`UdpReceiveError::Io`] for socket failures.
    pub fn recv_datagram(&mut self) -> Result<(UdpDatagram, SocketAddr), UdpReceiveError> {
        let (received, source) = self.socket.recv_from(&mut self.receive)?;
        if received > self.limits.max_datagram_bytes() {
            return Err(UdpReceiveError::Truncated {
        ...
        let datagram = decode_udp_datagram_with_limits(&self.receive[..received], self.limits)
            .map_err(UdpReceiveError::Decode)?;
        Ok((datagram, source))
    }
    ...
    /// accepted by the application. Decoding alone does not imply acceptance.
    pub fn send_ack_to(
        &self,
...
impl TeltonikaUdpSocket<tokio::net::UdpSocket> {
    ...
    pub async fn recv_datagram_async(
        &mut self,
    ) -> Result<(UdpDatagram, SocketAddr), UdpReceiveError> {
